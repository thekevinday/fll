// A config.h file for defining all code being disabled for compiling the stand alone Status Code.
// To assist in building this file use (may need to omit --defined-only): "nm --defined-only -f p programs/shared/status_code | grep -oP '^(f|fl|fll|macro)_[^\s]+'".
...
//
// Example: for i in f_type f_status f_memory f_type_array f_string f_utf f_color f_console f_conversion f_file f_pipe f_print f_signal f_status_string ; do grep -hor '#define _di_f_.*\>' level_0/$i >> /tmp/all.txt ; grep -hor '#define _di_macro_.*\>' level_0/$i >> /tmp/all.txt ; done

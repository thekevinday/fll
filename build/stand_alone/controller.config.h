// A config.h file for defining all code being disabled for compiling the stand alone Controller.
// To assist in building this file use (may need to omit --defined-only): "nm --defined-only -f p programs/shared/controller | grep -oP '^(f|fl|fll)_[^\s]+'".
...
//          for i in f_type f_status f_memory f_type_array f_string f_utf f_capability f_color f_console f_control_group f_conversion f_directory f_environment f_execute f_file f_fss f_iki f_path f_pipe f_print f_signal f_socket f_status_string f_thread ; do grep -horP '\b_di_f_\w*\b' level_0/$i/c >> /tmp/all.txt ; grep -horP '\b_di_macro_\w*\b' level_0/$i/c >> /tmp/all.txt ; done
...
#define _di_fl_directory_clone_file_
...
#define _di_fl_fss_basic_object_write
...
#define _di_fl_fss_extended_object_write
...
#define _di_fl_utf_file_write_range
...
#define _di_fl_utf_string__seek_to_character_
#define _di_fl_utf_string_seek_to_character_
#define _di_fl_utf_string_seek_to_utf_character_
...
#define _di_fll_status_string_to_

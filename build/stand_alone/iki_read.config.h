// A config.h file for defining all code being disabled for compiling the stand alone IKI Read.
// To assist in building this file use (may need to omit --defined-only): "nm --defined-only -f p programs/shared/iki_read | grep -oP '^(f|fl|fll|macro)_[^\s]+'".
...
//          for i in fl_conversion fl_iki fl_print fl_string ; do grep -horP '\b_di_fl_\w*\b' level_1/$i/c >> /tmp/all.txt ; grep -horP '\b_di_macro_\w*\b' level_1/$i/c >> /tmp/all.txt ; done

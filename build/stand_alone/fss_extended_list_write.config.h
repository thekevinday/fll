// A config.h file for defining all code being disabled for compiling the stand alone FSS Extended List Write.
// To assist in building this file use (may need to omit --defined-only): "nm --defined-only -f p programs/shared/fss_extended_list_write | grep -oP '^(f|fl|fll|macro)_[^\s]+'".
...
//          for i in f_type f_status f_memory f_type_array f_string f_utf f_color f_console f_conversion f_directory f_environment f_file f_fss f_pipe f_print f_signal ; do grep -horP '\b_di_f_\w*\b' level_0/$i/c >> /tmp/all.txt ; grep -horP '\b_di_macro_\w*\b' level_0/$i/c >> /tmp/all.txt ; done
//          for i in fl_conversion fl_fss fl_print fl_string ; do grep -horP '\b_di_fl_\w*\b' level_1/$i/c >> /tmp/all.txt ; grep -horP '\b_di_macro_\w*\b' level_1/$i/c >> /tmp/all.txt ; done
//          for i in fll_error fll_file fll_fss fll_print fll_program ; do grep -horP '\b_di_fll_\w*\b' level_2/$i/c >> /tmp/all.txt ; grep -horP '\b_di_macro_\w*\b' level_2/$i/c >> /tmp/all.txt ; done

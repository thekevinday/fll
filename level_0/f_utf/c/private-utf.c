  ...
  f_return_status private_f_utf_character_is_valid(const f_utf_character character, const uint8_t width) {
    ...
    if (width == 2) {
      ...
      if (byte_first == 0xe0) {
        ...
        // consider all private use codes as valid, U+E000 to U+EFFF (U+F8FF).
        ...
        // consider all private use codes as valid, U+F000 (U+E000) to U+F8FF.
        if (bytes >= 0x8080 && bytes <= 0xa3bf) {
          return F_true;
        }

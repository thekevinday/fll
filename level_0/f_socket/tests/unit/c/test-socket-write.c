...
void test__f_socket_write__fails(void **state) {
  ...
  for (uint8_t i = 0; i < 22; ++i) {
    ...
    will_return(__wrap_sendto, true);
    will_return(__wrap_sendto, errnos[i]);
...
void test__f_socket_write__parameter_checking(void **state) {
  ...
  {
    const f_status_t status = f_socket_write(&socket, 0, 0, 0);

    assert_int_equal(status, F_status_set_error(F_parameter));
  }

  {
    const f_status_t status = f_socket_write(0, 0, buffer, 0);
...
void test__f_socket_write__works(void **state) {
  ...
  {
    will_return(__wrap_sendto, false);

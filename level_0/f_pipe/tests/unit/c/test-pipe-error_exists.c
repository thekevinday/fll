...
void test__f_pipe_error_exists__fails(void **state) {
  ...
  for (int i = 0; i < 11; ++i) {
    ...
    const f_status_t status = f_pipe_error_exists();

    assert_int_equal(status, F_status_set_error(F_file_stat));
  } // for
}

void test__f_pipe_error_exists__returns_false(void **state) {
...
void test__f_pipe_error_exists__returns_true(void **state) {

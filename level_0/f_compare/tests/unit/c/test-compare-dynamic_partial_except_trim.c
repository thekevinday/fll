...
void test__f_compare_dynamic_partial_except_trim__works(void **state) {
  ...
  for (; i < 8; ++i) {
    ...
    if (i % 8 == 0) {
      ...
      status = f_compare_dynamic_partial_except_trim(string_1s[i], string_2s[i], range_1, range_2, excepts_1, excepts_2);
    }

...
extern "C" {
  ...
  f_status_t status_code_print_help(const f_file_t file, const f_color_context_t context) {
    ...
    fl_print_format("  These codes stop at the %[F_status_code_last%] code.%r%r", file.stream, context.set.notable, context.set.notable, f_string_eol_s, f_string_eol_s);

    fl_print_format("  Each code will be printed on its own line.%r", file.stream, context.set.notable, context.set.notable, f_string_eol_s);
